...
params.STIP_subsample_num=1000; % Subsampling number of STIP features per video clip

params.encoding='fisher'; % encoding type: 'fisher' - fisher vector; 'bow' - bag-of-words
params.feat_list={'HOG','HOF'}; % all features involved in this test
feat_len={72,90}; % length of features
...
params.train_sample_data=fullfile(path,'data',sprintf('UCF101_train_data_sample%d_gmm%d.mat',params.STIP_subsample_num,params.K));
params.test_sample_data=fullfile(path,'data',sprintf('UCF101_test_data_sample%d_gmm%d.mat',params.STIP_subsample_num,params.K));

% Files to store Fisher vectors
params.fv_train_file=fullfile(path,'data',sprintf('pca_gmm_data_train_sample%d_gmm%d.mat',params.STIP_subsample_num,params.K));
...
switch params.encoding
      case 'fisher'        
        ...
        % Subsample STIP features, calculate PCA coefficients and train GMM model
        ...
        parfor i=1:numel(uniq_labels)
			...
			save_file=sprintf('fisher_vectors_action%d_sample%d_gmm%d.mat',i,params.STIP_subsample_num,params.K);
			parsave(fullfile(path,'data',save_file),fvt_pos_train,fvt_neg_train, fvt_pos_test, fvt_neg_test);
        end
    ...
    case 'bow'
        ...
        cb_size=4000;
        redo_num=8;
        [Chog, Dhog, Ihog] = yael_kmeans(single(stip_train{1}),cb_size,'redo', redo_num);
        [Chof, Dhof, Ihof] = yael_kmeans(single(stip_train{2}),cb_size,'redo', redo_num);
        [Cmbhx, Dmbhx, Imbhx] = yael_kmeans(single(stip_train{3}),cb_size,'redo', redo_num);
        [Cmbhy, Dmbhy, Imbhy] = yael_kmeans(single(stip_train{4}),cb_size,'redo', redo_num);
        
        hog_cb=zeros(cb_size,size(stip_train{1},2));
        hof_cb=zeros(cb_size,size(stip_train{2},2));
        mbhx_cb=zeros(cb_size,size(stip_train{3},2));
        mbhy_cb=zeros(cb_size,size(stip_train{4},2));
        % Vector Quantization
        for i=1:size(stip_train{1},2)
            % TBD %
        end
